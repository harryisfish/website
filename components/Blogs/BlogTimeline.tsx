...
export function BlogTimeline({ blogs }: BlogTimelineProps) {
  ...
  const timelineData = sortedYears.map(year => {
    ...
    return {
      ...
      content: (
        <div>
          <p className="mb-12 text-lg font-bold text-neutral-800 dark:text-neutral-200">
          ...
          <div className="space-y-6">
            {yearBlogs.map((blog) => (
              ...
              >
                <div>
                  <h3 className="text-xl font-bold text-black dark:text-white group-hover:text-neutral-400 dark:group-hover:text-neutral-500 transition-colors">
                    {blog.title}
                  </h3>
